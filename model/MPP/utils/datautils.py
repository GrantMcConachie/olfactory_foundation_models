...
class SMILESProteinDataset(Dataset):
    def __init__(self,  
        ...
        self.embed_dir = os.path.dirname(os.path.dirname(embed_dir))
        
        self.df = pd.read_csv(join(data_path))
        self.prot_dicts = os.listdir(join(self.embed_dir, "featurized_proteins"))
        self.smiles_dicts = [embed_dir]
    ...
    def update_subset(self):
        ...
        self.protein_repr = self._load_protein_repr(join(self.embed_dir, "featurized_proteins", self.prot_dicts[self.protein_subset_no]))

        if self.protein_subset_no == 0:
            smiles_repr_file = join(self.embed_dir, "featurized_mols", self.smiles_dicts[self.smiles_subset_no])
