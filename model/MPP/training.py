...
def train(args, model, trainloader, optimizer, criterion, device, gpu, epoch):
    ...
    for step, batch in enumerate(trainloader):
        ...
        smiles_emb, smiles_attn, protein_emb, protein_attn, labels, _, prots, smis = batch
        smiles_emb.cuda(gpu)
        smiles_attn.cuda(gpu)
        protein_emb.cuda(gpu)
        protein_attn.cuda(gpu)
        labels.cuda(gpu)
        _.cuda(gpu)
        ...
        if "M2OR_full" in args.train_dir:
            loss = criterion(outputs, labels.float(), smis, prots)
        else:
...
def trainer(gpu, args, device, split):
    ...
    logger.info(f'smiles dimesion size: {dim_size}')
    ...
    if args.binary_task:
        ...
        # implementing the weighted cross entropy loss
        if "M2OR_full" in args.train_dir:
